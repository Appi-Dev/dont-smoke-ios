...
The manual build workflow is `.github/workflows/testflight-poc.yml`. It builds QA version **9.0.0**, exports an App Store Connect signed IPA, uploads it with Xcode’s `altool --upload-package`, waits for processing, and configures a separate external TestFlight group for each build and its configured testers. It never creates a production App Store version, submits production App Review, or releases an App Store version. The only review submission is **Beta App Review**. IPA creation runs in parallel; uploads and external distribution run independently.

## Separate 9.0.1 upload experiment

`.github/workflows/testflight-poc-9-0-1.yml` adds **Actions → External TestFlight POC 9.0.1**. It uses the same signing secrets, tester secret, app record, Xcode, encrypted artifacts, processing checks, and external distribution logic. Its workflow-wide `TF_VERSION` is fixed to `9.0.1`; the existing QA workflow explicitly selects `9.0.0`. Local Xcode project version settings are not changed by this configuration.

Both workflows share the existing reservation ledger and monotonically increasing integer counter. If QA reserved 21, the next 9.0.1 run reserves 22, and the next QA run reserves 23. There is no separate counter to initialize or reset. Leave `initialize_state` **false** when the existing ledger is present. Reservation identity, version, source revision, and authenticated IPA metadata are verified. Groups are generated from the selected version/build, such as `9.0.1 (22)`.

After committing and merging into the default branch, manually dispatch the **9.0.1** workflow to test whether Apple accepts an upload to the same app under a different marketing version. Compare its Upload result and Apple error codes with the QA run. This is an experiment, not a guaranteed bypass of error 90382. A successful upload tests upload acceptance only; this workflow still distributes through external TestFlight and never submits production App Review or releases an App Store version. The offline suite runs with the QA default plus explicit tests for 9.0.1 configuration, group naming, processing lookup, and shared numbering.

The QA workflow automatically dispatches the 9.0.1 workflow **only** when upload or Build Uploads processing reports numeric error `90382`. It requires `actions: write` on the QA submission job; organisation policy must permit it. A conditional ledger write claims the fallback once before dispatch. Dispatch errors or uncertain responses are not retried; inspect Actions before manually intervening. The original QA run remains failed, and its summary records dispatch status. GitHub dispatch acceptance does not imply the fallback completed. The 9.0.1 workflow never dispatches another fallback. Both workflows must be present on the default branch before enabling this flow; no fallback is attempted for timeouts, signing failures, or unrelated errors. The new run reserves a fresh number and rebuilds as 9.0.1. Dispatch does not return a run ID, so the summary directs you to Actions runs rather than inventing a child-run link.
...
3. **submit (Mac):** starts as soon as its own build job succeeds. It downloads only its run/attempt artifact, verifies authentication, metadata/source revision and IPA checksum, then checks Apple state, then atomically claims its own reservation before upload or resume. The claim prevents duplicate upload of that reservation; it does not wait for any other reservation. Each run uploads, waits for its own processing, creates or reuses its generated group, and configures external TestFlight independently. Successful configuration marks it completed, including explicitly pending Beta App Review. Failures after claiming retain the legacy `blocked` status for operator reconciliation of that reservation only; other runs continue.

For example, IPA 22 can upload before IPA 21 finishes building. Both can upload and process simultaneously. Failed, cancelled, or uncertain reservations never hold other runs. Their reservations remain available to their own reruns. After pushing a workflow fix, dispatch a **new run** so GitHub uses the updated commit; rerunning an older run uses its original code. With updated code, either rerun failed jobs or all jobs: both retain the original reservation. A new manual dispatch is a new logical build and gets a new number.
...
Initialise this new branch only for its first run, after finishing older pipelines. Preserve any older state branch for history; do not reset it. A failed run retains its number for retries; parallel or permanently abandoned runs can still leave gaps. Allocation fails above 9999. Do not delete, reset, manually edit, or force-push the new state branch. Restrict modifications and retain its Git history.
...
Reservation has a 15-minute job limit, IPA creation 70 minutes, upload/processing/distribution 135 minutes. Apple [Build Uploads status](https://developer.apple.com/documentation/appstoreconnectapi/get-v1-apps-_id_-builduploads) is checked by exact app, marketing version, build and IOS platform alongside processed-build lookup. A `FAILED` upload immediately fails processing, even when no `/v1/builds` record exists. Only numeric structured error codes are reported; descriptions remain private. Empty upload results keep polling, and `COMPLETE` still requires the matching build to become `VALID`. Permission errors, ambiguous records, unexpected states, and identity mismatches fail closed. Apple processing is polled approximately every 30 seconds for up to 90 minutes with bounded read retries. `FAILED`/`INVALID` processing fails the job. Beta App Review approval is not polled indefinitely; pending configuration is explicitly reported.

## Retry and resume behavior

A reservation now belongs to the GitHub **run ID**, rather than each retry attempt. Its original `run_id.attempt` identifier remains stable for compatibility with encrypted metadata and recovery. New attempts for that same source revision and marketing version reuse the number. Existing runs with multiple legacy reservations fail closed rather than guessing which number to reuse. Old runs execute their old workflow code; deploy this change before creating runs that need this retry behavior. Existing gaps are not automatically reclaimed.

A successful build records its artifact attempt in the ledger. Rerunning only submission downloads that earlier encrypted IPA. Rerunning a failed build or all jobs creates a new artifact under the same reserved number. Encrypted artifacts are retained for seven days; if expired, rerun all jobs to rebuild. Before submission, Apple is checked for the exact app/version/build/platform. Existing processed builds or processing/complete uploads resume distribution without another upload. Confirmed failed uploads can be retried with the same number; earlier failed delivery records are excluded from the new processing wait. A prior upload with no visible Apple record remains uncertain and is refused. A persisted explicit upload-limit rejection also permits retry. API lookup errors fail closed. Submission claims are tracked per attempt and cannot be replayed or moved backward.

Parallel builds remain independent: a later number may finish before an earlier failed build is retried. Apple still decides whether to accept the older number. Permanently abandoned runs or new manual dispatches can leave gaps; strict gap-free ordering is not guaranteed. The 9.0.1 fallback remains a separate run/version with its own reservation, and its once-only dispatch claim survives retries.

## Encrypted IPA transfer

The jobs run on separate machines, so the IPA must be transferred through a GitHub artifact. Only `ipa.bundle` is published, retained for seven days. It contains an AES-256-CBC encrypted ZIP of the IPA and sanitized metadata, using OpenSSL PBKDF2 (200,000 iterations) with a random salt. An independent salted PBKDF2-derived HMAC-SHA256 key authenticates the container before decryption. Metadata and IPA checksums are checked against the reservation. Tampered artifacts or a different run/revision are refused.

The existing `KEYCHAIN_PASSWORD` secret is also used as the transfer passphrase. Use a strong random value; keep it unchanged during active runs, and rotate it when those runs/artifacts have finished or expired. Plaintext IPAs, archives, keys, profiles, certificates, and tester payloads are never artifacts. A dedicated transfer secret can replace the reused passphrase in a future revision if separate rotation is needed.

## Failure handling and privacy

Raw signing, Xcode, upload-tool, and API output is deliberately suppressed because it can contain credentials or identities. Errors report the failed tool/HTTP status without response bodies or query strings. Failed IPA uploads additionally extract only five-digit Apple error codes from explicit `ITMS-`, `ERROR:`, or parenthesized error markers (for example `90382`). Codes appear in the sanitized upload result and failure summary; messages and raw output remain suppressed. Unrecognized nonzero failures report only the tool exit status. Upload timeouts remain uncertain and are not automatically retried. Requests cannot redirect credentials to a different host. No plaintext IPA, archive, P8, profile, certificate, raw diagnostic output, or tester payload is published as a workflow artifact. The authenticated encrypted IPA artifact is the only transfer artifact.
