...
Build numbers are sequential integers: the allocator reads all uploaded builds for version 9.0.0, starts above a floor of 20, and chooses the smallest integer greater than the largest uploaded build. With build 20 as the latest upload, runs produce 21, 22, 23. Reruns after an upload increment again; a run that fails before upload can reuse its unuploaded number. Previously uploaded numbers, including expired builds, are never ignored. Because the POC already uploaded 9000.1.2 for version 9.0.0, the next safe integer for that version is 9001, followed by 9002. Starting at 21 requires a new marketing version approved by the repository owner; the workflow does not change marketing version automatically. Integer allocation fails above 9999 rather than silently changing formats.

GitHub workflow concurrency serializes runs without interrupting an upload. GitHub can replace older pending runs with newer dispatches; pending runs are not a durable queue. Other upload pipelines must coordinate separately: this is not an atomic cross-pipeline build-number reservation.
